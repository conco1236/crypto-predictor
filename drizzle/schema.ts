...
export const technicalAiSettings = mysqlTable("technical_ai_settings", {
  ...
  quotaAlertEnabled: int("quotaAlertEnabled").default(1).notNull(),
  quotaAlertThresholdPercent: int("quotaAlertThresholdPercent").default(10).notNull(),
  quotaAlertState: mysqlEnum("quotaAlertState", ["normal", "low", "unavailable"]).default("unavailable").notNull(),
  quotaAlertKind: varchar("quotaAlertKind", { length: 30 }),
  quotaAlertDeliveryStatus: mysqlEnum("quotaAlertDeliveryStatus", ["pending", "sent", "failed"]).default("pending").notNull(),
  quotaCheckLastAt: timestamp("quotaCheckLastAt"),
  quotaAlertLastAt: timestamp("quotaAlertLastAt"),
  quotaAlertLastPercent: int("quotaAlertLastPercent"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
