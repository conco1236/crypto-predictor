...
export default function IntegrationSettings() {
  ...
  const testAiConnection = trpc.news.testTechnicalAiConnection.useMutation({
    onSuccess: result => toast.success(`Manual API hoạt động · ${result.model} · ${result.latencyMs}ms`),
    onError: error => toast.error(error.message),
  });
  ...
  const submitAi = () => saveAi.mutate({ mode: aiMode, model: aiModel.trim(), ...(aiMode === "manual_api" ? { apiBaseUrl: apiBaseUrl.trim(), ...(apiKey.trim() ? { apiKey: apiKey.trim() } : {}) } : {}) });
