...
export function formatSignalAlert(analysis: MarketAnalysis, aiAnalysis?: string, news: NewsItem[] = []) {
  ...
  return [
    ...
    ...(analysis.signalQuality && analysis.signalQuality.penalty >= 20 ? [`<b>⚠️ Quality alert:</b> confidence giảm ${analysis.signalQuality.penalty} điểm do ${escapeTelegramHtml(analysis.signalQuality.reasons.slice(0, 2).join("; "))}`] : []),
