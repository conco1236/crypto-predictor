...
import { getLastSignal, getProcessedCandle, getTelegramSettingsByTaskUid, markProcessedCandle, saveSignalSnapshot } from "../db";
...
export async function refreshSignalsHandler(req: Request, res: Response) {
  ...
  try {
    ...
    for (const a of analyses) {
      const processed = await getProcessedCandle(settings.userId, a.exchange, a.symbol, a.interval);
    ...
    return res.json({ ok: true, saved, alerts, durationMs: Date.now() - startedAt });
  } catch (error) {
