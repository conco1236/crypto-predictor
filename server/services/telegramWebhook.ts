...
import { answerTelegramCallbackQuery, buildPaperTradeInlineKeyboard, sendTelegramMessage } from "./telegram";
...
export async function handleTelegramPaperWebhook(update: TelegramUpdate) {
  ...
  const rawText = callback?.data ? callback.data.replace(/^paper:/, "/paper_").replace(/:/g, " ") : (update.message?.text ?? "").trim();
  ...
  if (name === "/paper_pause" || name === "/paper_resume") {
    ...
    if (!analysis || analysis.signalStatus === "No Trade") reply = "Không tìm thấy tín hiệu Trade phù hợp; không mở paper trade.";
      ...
      tradeId = trade?.id;
      await createPaperBotAudit(settings.userId, "paper_open_telegram", `Mở ${direction} ${analysis.symbol} từ ${callback ? "inline keyboard" : "Telegram"}; không phải lệnh live`);
      reply = `Đã mở paper trade #${tradeId ?? "—"} ${direction} ${analysis.symbol} ${analysis.interval}. Entry ${analysis.levels.entry.toFixed(2)} · TP ${analysis.levels.takeProfit1.toFixed(2)} · SL ${analysis.levels.stopLoss.toFixed(2)}.`;
    }
    ...
    else { await updatePaperTrade(trade.id, settings.userId, { currentPrice: trade.currentPrice, status: "cancelled", closedAt: Date.now(), exitPrice: trade.currentPrice, pnlPercent: ((trade.currentPrice - trade.entry) / trade.entry) * (trade.direction === "Long" ? 100 : -100) }); await createPaperBotAudit(settings.userId, "paper_close_telegram", `Đóng paper trade #${trade.id} từ inline keyboard`); reply = `Đã đóng paper trade #${trade.id}. Không có lệnh live được gửi.`; }
  }
  await sendTelegramMessage(settings.botToken, chatId, reply, callback ? buildPaperTradeInlineKeyboard(tradeId, isPaperBotPaused(settings.userId)) : undefined);
  if (callback?.id) await answerTelegramCallbackQuery(settings.botToken, callback.id, reply.slice(0, 180));
