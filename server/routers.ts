...
export const appRouter = router({
  ...
  market: router({
    ...
    momentumCriticalAlertHistory: protectedProcedure.input(z.object({ limit: z.number().int().min(1).max(100).default(12) }).optional()).query(({ ctx, input }) => getMomentumCriticalAlertHistory(ctx.user.id, input?.limit ?? 12)),
