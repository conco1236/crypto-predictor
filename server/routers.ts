...
export const appRouter = router({
  ...
  paper: router({
    list: protectedProcedure.input(z.object({ limit: z.number().int().min(1).max(200).default(100) }).optional()).query(({ ctx, input }) => getPaperTrades(ctx.user.id, input?.limit ?? 100)),
    audit: protectedProcedure.input(z.object({ limit: z.number().int().min(1).max(200).default(50) }).optional()).query(({ ctx, input }) => getPaperBotAudit(ctx.user.id, input?.limit ?? 50)),
    auditEvent: protectedProcedure.input(z.object({ action: z.string().min(1).max(60), detail: z.string().min(1).max(1000) })).mutation(async ({ ctx, input }) => { await createPaperBotAudit(ctx.user.id, input.action, input.detail); return { ok: true }; }),
    mockCredentialStatus: protectedProcedure.input(z.object({ exchange: z.enum(["Binance", "OKX"]) })).query(({ input }) => getMockCredentialStatus(input.exchange as MockExchange)),
    dryRun: protectedProcedure.input(z.object({ exchange: z.enum(["Binance", "OKX"]), symbol: z.enum(["BTCUSDT", "ETHUSDT"]), side: z.enum(["Long", "Short"]), entry: z.number().positive(), takeProfit: z.number().positive(), stopLoss: z.number().positive(), currentPrice: z.number().positive(), quantity: z.number().positive(), maxRiskPercent: z.number().positive().max(5) })).mutation(async ({ ctx, input }) => { const result = validateDryRunOrder(input); await createPaperBotAudit(ctx.user.id, "dry_run", `${result.ok ? "Đạt" : "Từ chối"} ${input.side} ${input.symbol} trên mock ${input.exchange}; Withdraw=false; ${result.projectedRiskPercent.toFixed(3)}% risk`); return result; }),
    create: protectedProcedure.input(z.object({ exchange: z.string().min(1), symbol: z.string().min(1), interval: z.string().min(1), direction: z.enum(["Long", "Short"]), entry: z.number(), takeProfit: z.number(), stopLoss: z.number(), currentPrice: z.number(), openedAt: z.number(), sourceSignalKey: z.string().min(1) })).mutation(async ({ ctx, input }) => { const trade = await createPaperTrade({ userId: ctx.user.id, ...input }); await createPaperBotAudit(ctx.user.id, "paper_trade", `Mở ${input.direction} ${input.symbol} ${input.interval} từ tín hiệu thật`); return trade; }),
    refresh: protectedProcedure.mutation(async ({ ctx }) => { if (isPaperBotPaused(ctx.user.id)) return { updated: 0, closed: 0, paused: true, refreshedAt: Date.now() }; const [trades, analyses] = await Promise.all([getPaperTrades(ctx.user.id, 200), analyzeAllMarkets()]); const byKey = new Map(analyses.map(a => [`${a.exchange}:${a.symbol}:${a.interval}`, a.price])); let closed = 0; for (const trade of trades.filter(item => item.status === "open")) { const price = byKey.get(`${trade.exchange}:${trade.symbol}:${trade.interval}`); if (price == null) continue; let status: "open" | "take_profit" | "stop_loss" = "open"; if (trade.direction === "Long") { if (price >= trade.takeProfit) status = "take_profit"; else if (price <= trade.stopLoss) status = "stop_loss"; } else { if (price <= trade.takeProfit) status = "take_profit"; else if (price >= trade.stopLoss) status = "stop_loss"; } const pnlPercent = ((price - trade.entry) / trade.entry) * (trade.direction === "Long" ? 100 : -100); await updatePaperTrade(trade.id, ctx.user.id, { currentPrice: price, status, pnlPercent, ...(status === "open" ? {} : { closedAt: Date.now(), exitPrice: price }) }); if (status !== "open") { closed++; await createPaperBotAudit(ctx.user.id, "paper_exit", `${status === "take_profit" ? "TP" : "SL"} ${trade.symbol} tại ${price.toFixed(2)}`); } } return { updated: trades.length, closed, refreshedAt: Date.now() }; }),
  }),
  market: router({
    ...
    confidenceHistory: protectedProcedure.input(z.object({ exchange: z.enum(["Binance", "Bybit", "OKX"]), symbol: z.enum(["BTCUSDT", "ETHUSDT"]), interval: z.enum(["15m", "1h", "4h", "1d"]), limit: z.number().int().min(2).max(60).default(36) })).query(({ ctx, input }) => getConfidenceHistory(ctx.user.id, input.exchange, input.symbol, input.interval, input.limit)),
    confidenceEarlyWarnings: protectedProcedure.input(z.object({ limit: z.number().int().min(1).max(30).default(12) }).optional()).query(({ ctx, input }) => getConfidenceEarlyWarnings(ctx.user.id, input?.limit ?? 12)),
    momentumSettings: protectedProcedure.query(({ ctx }) => getMomentumSettings(ctx.user.id)),
    saveMomentumSettings: protectedProcedure.input(z.object({ criticalDropThreshold: z.number().int().min(6).max(50), deterioratingDropThreshold: z.number().int().min(3).max(30) }).refine(input => input.criticalDropThreshold > input.deterioratingDropThreshold, { message: "Ngưỡng Critical phải lớn hơn Deteriorating" })).mutation(({ ctx, input }) => saveMomentumSettings(ctx.user.id, input)),
